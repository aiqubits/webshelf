...
**注**：原型中仅此一种按钮样式（`.btn-gradient`）。暂不需要 secondary / text-arrow / icon-circular 变体。如需扩展，参考 DESIGN-DEMO.md 组件体系。
...
- **不要引入 DESIGN-DEMO.md 的色阶强调色体系**（`accent-purple/pink/blue/orange/green`）。
...
*本文档即 `app/ui` crate 的 UI 设计权威提示词。所有 Dioxus 组件的视觉实现必须严格对齐本文档的设计 token 与组件规格。事实源为 `frontend-design/prototype.html`，DESIGN-DEMO.md 仅作为提示词格式参考。*
